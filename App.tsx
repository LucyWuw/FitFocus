...
const App: React.FC = () => {
  ...
  const [aiAdvice, setAiAdvice] = useState<string>("Hop on in! Coach Kanga is ready to help you hit those goals!");
  ...
  const getIndicatorColor = (pct: number) => {
    if (pct >= 90 && pct <= 110) return 'text-emerald-500 font-black';
    if (pct < 50 || pct > 130) return 'text-rose-500 font-black';
  ...
  const copyDailySummaryToClipboard = () => {
    ...
    navigator.clipboard.writeText(summary).then(() => {
      ...
      confetti({ particleCount: 80, spread: 40, origin: { y: 0.1 } });
    });
  };
    ...
    <div className="max-w-5xl mx-auto px-4 py-8 md:py-12">
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8 no-print">
        ...
        <div className="flex flex-wrap gap-2">
          <button onClick={copyDailySummaryToClipboard} className={`px-6 py-3 rounded-2xl font-bold transition-all flex items-center gap-2 border-2 ${copyFeedback ? 'bg-emerald-500 border-emerald-500 text-white' : 'bg-white border-slate-200 text-slate-700 hover:border-indigo-500 hover:text-indigo-600 shadow-sm'}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-1 space-y-6">
          ...
          <section className="glass-card p-6 rounded-3xl border border-white no-print">
            <h2 className="text-lg font-bold mb-4 flex items-center gap-2"><i className="fas fa-comment-dots text-indigo-500"></i> Fast Local Log</h2>
            <form onSubmit={handleAddEntry} className="space-y-4">
              <textarea
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                placeholder="Try: 'Ate 400 calories of chicken with 30g protein' or '30 min run burning 300 kcal'"
              ...
              <button type="submit" disabled={isAnalyzing || !inputText.trim()} className="w-full py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all flex items-center justify-center gap-2">
                {isAnalyzing ? <i className="fas fa-spinner fa-spin"></i> : <i className="fas fa-keyboard"></i>}
                {isAnalyzing ? 'Processing...' : 'Add Record'}
              </button>
            </form>
          </section>
        </div>
          ...
          <section className="glass-card p-6 md:p-8 rounded-[2.5rem] shadow-sm min-h-[500px] border border-white/50">
            <div className="flex items-center justify-between mb-8">
              <div>
                ...
                <p className="text-sm text-slate-400 font-medium italic">Double-click numbers to correct values</p>
              </div>
            ...
            <LogList entries={entries} onDelete={id => setEntries(prev => prev.filter(e => e.id !== id))} onUpdateEntry={handleUpdateEntry} />
          </section>
        </div>
      </div>
    </div>
